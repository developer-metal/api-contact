...
import { SanitizeHtml } from "src/form-reception/validators/sanitizeHtml.constraint";
...
@SanitizeHtml({ message: 'HTML -name- no permitido.'})
name: string;
...
@SanitizeHtml({ message: 'HTML -email- no permitido.'})
email: string;
...
@SanitizeHtml({ message: 'HTML -project- no permitido.'})
project: string;
}